...
public class PFact
{
    static public void Main ()
    {
        ...
        Console.WriteLine(sqrtlimit);

        long potential_prime           = 2;
        List <long> skip_set           = new List <long>();
        skip_set.Add(potential_prime);
        List <long> eliminate          = new List <long>();
        eliminate.Add(potential_prime);
        potential_prime += 1;
        List <long> skip_range         = new List <long>();
        skip_range.Add(skip_set[0]);
        List <long> verified_primes    = new List <long>();
        verified_primes.Add(potential_prime);
        long current                   = 0;
        skip_range.Add(skip_set[(int)current]*verified_primes[0]);
        List <long> extended_primes    = new List <long>();
        potential_prime += skip_set[0];
        long next_potential_prime      = potential_prime;
        verified_primes.Add(next_potential_prime);
        while (next_potential_prime < limit){
            current++;
